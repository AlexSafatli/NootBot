...
public class ReportSoundProcessor extends SingleArgumentChatCommandProcessor {
  ...
  public static final SimpleLog LOG = SimpleLog.getLog("ReportSound");
  ...
  protected void handleEvent(MessageReceivedEvent event, String message) {
    ...
    if (name == null) {
      ...
      String suggestion = "Check your spelling.", possibleName =
              bot.getClosestMatchingSoundName(name);
      ...
      if (bot.getUser(user).getSoundsReported().contains(name)) {
        ...
        pm(event, "You already reported this sound.");
        return;
      } else {
        LOG.info("Adding " + name + " to sounds that the user " +
                user.getName() + " reported on.");
      ...
      pm(event, StyledEmbedMessage.forSoundFile(bot, file,
              "Information for Sound `" + file.getSoundFileId() + "`",
              "You have successfully reported the sound called `" +
                      file.getSoundFileId() + "`."));
      if (file.getNumberOfReports() >= NUMBER_OF_REPORTS_FOR_DELETE &&
              delete(file)) {
        pm(event, "Your report was enough to have the sound get deleted. " +
                "You won't have to deal with `" + name + "` anymore.");
        bot.sendMessageToUser("Deleted `" + name +
                        "` because of too many reports but made a copy " +
        ...
        bot.getDispatcher().saveSound(file);
        bot.sendMessageToUser("Excluded `" + name +
                        "` from random because of reports. Last report " +
                        "was made by **" + user.getName() + "**.",
                bot.getOwner());
  ...
  private boolean delete(SoundFile file) {
    try {
      Files.copy(file.getSoundFile(), bot.getTempPath().resolve(
              file.getSoundFile().getName()).toFile());
  ...
  public String getCommandHelpString() {
    return getPrefix() +
            " <soundfile> - report a sound file for being inappropriate or " +
            "annoying";
  }
}